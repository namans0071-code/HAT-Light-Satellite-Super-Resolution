...
def main():
    ...
    ds = SatelliteSRDataset("cross_sensor/data", scale=4, is_train=False, augment=False)
    ...
    # Save master JSON
    out_json = Path("cross_sensor/cross_sensor_results.json")
    ...
    out_md = Path("cross_sensor/table_cross_sensor.md")
    out_md.write_text("\n".join(md_lines))
    ...
    for name, r in results.items():
        ...
        gain = r["psnr_overall"] - bic_psnr
        g_str = f"+{gain:.2f} dB" if gain > 0 else "0.00 dB"
        tex_lines.append(f"{name} & {b(r['psnr_overall'])} & {b(r['ssim'])} & {b(r['sam_deg'])} & {b(r['ergas'])} & {b(g_str)} \\\\")
    tex_lines.extend([
        "\\hline",
        "\\end{tabular}",
        "\\end{table}"
    ])
    out_tex = Path("paper/tables/tab3_cross_sensor.tex")
    out_tex.write_text("\n".join(tex_lines))
