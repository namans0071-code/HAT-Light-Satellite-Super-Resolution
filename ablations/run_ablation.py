...
def main():
    ...
    out_json = Path("ablations/ablation_results.json")
    ...
    for name, r in ablation_matrix.items():
        ...
        b = lambda s: f"\\textbf{{{s}}}" if is_full else str(s)
        tex_lines.append(f"{name} & {b(r['psnr'])} & {b(r['ssim'])} & {b(r['sam'])} & {b(r['ergas'])} \\\\")
    tex_lines.extend([
        "\\hline",
        "\\end{tabular}",
        "\\end{table}"
    ])
    out_tex = Path("ablations/table_ablation.tex")
    out_tex.write_text("\n".join(tex_lines))
    print(f"Saved LaTeX ablation table to {out_tex}")
    ...
    out_md = Path("ablations/table_ablation.md")
    out_md.write_text("\n".join(md_lines))
