...
def main():
    ...
    if edsr_path.exists():
        ...
        ckpt_e = torch.load(str(edsr_path), map_location=device, weights_only=False)
    ...
    if rcan_path.exists():
        ...
        ckpt_r = torch.load(str(rcan_path), map_location=device, weights_only=False)
    ...
    if swin_path.exists():
        ...
        ckpt_s = torch.load(str(swin_path), map_location=device, weights_only=False)
    ...
    if hat_path.exists():
        ...
        ckpt_h = torch.load(str(hat_path), map_location=device, weights_only=False)
